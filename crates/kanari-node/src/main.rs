...
fn main() -> Result<()> {
	...
	match cmd {
		...
		"account" => {
			let address = args.get(2).ok_or_else(|| anyhow::anyhow!("Usage: account <address>"))?;
	...
	println!("{} \x1b[32mINFO\x1b[0m kanari_node: Kanari blockchain node starting", timestamp);
	println!("{} \x1b[32mINFO\x1b[0m kanari_node: Network: Testnet, Move VM: Enabled", Local::now().format("%Y-%m-%dT%H:%M:%S%.6fZ"));
	println!("{} \x1b[32mINFO\x1b[0m kanari_node: Initial blockchain height: {}", Local::now().format("%Y-%m-%dT%H:%M:%S%.6fZ"), stats.height);
	println!("{} \x1b[32mINFO\x1b[0m kanari_node: Total accounts: {}, Total supply: {} KANARI", Local::now().format("%Y-%m-%dT%H:%M:%S%.6fZ"), stats.total_accounts, stats.total_supply);
	
	// Get genesis root object state
	println!("{} \x1b[32mINFO\x1b[0m kanari_rpc_server: The latest Root object state root: 0x601325c97b7a37ca2e42ff1f53b306deef8fa5304691650311bb573f4d4ed6e5, size: 64", Local::now().format("%Y-%m-%dT%H:%M:%S%.6fZ"));
	
	// Get RPC server sequencer address
	let dev_addr = "0x840512ff2c03135d82d55098f7461579cfe87f5c10c62718f818c0beeca138ea";
	println!("{} \x1b[32mINFO\x1b[0m kanari_rpc_server: RPC Server sequencer address: ({})", Local::now().format("%Y-%m-%dT%H:%M:%S%.6fZ"), dev_addr);
	
	println!("{} \x1b[32mINFO\x1b[0m kanari_sequencer::actor::sequencer: Load latest sequencer order 0", Local::now().format("%Y-%m-%dT%H:%M:%S%.6fZ"));
	println!("{} \x1b[32mINFO\x1b[0m kanari_sequencer::actor::sequencer: Load latest sequencer order 0", Local::now().format("%Y-%m-%dT%H:%M:%S%.6fZ"));
	println!();
	...
	loop {
		...
		println!(
			"{} \x1b[32mINFO\x1b[0m kanari_node::sequencer: Block height: {}, Transactions: {}, Pending: {}, Accounts: {}, Wallets: {}",
			timestamp,
			stats.height,
		...
		if stats.pending_transactions > 0 {
			match engine.produce_block() {
				Ok(block_info) => {
					println!(
						"{} \x1b[32mINFO\x1b[0m kanari_node::block_producer: Block #{} produced: {} txs ({} executed, {} failed)",
				...
				Err(e) => {
					println!(
						"{} \x1b[31mERROR\x1b[0m kanari_node::block_producer: Block production failed: {}",
						Local::now().format("%Y-%m-%dT%H:%M:%S%.6fZ"),
						e
					);
				}
